...
# Usage ./pgconfig.sh 1024 2  etcpostgresql9.1mainpostgresql.conf


if [ $# -lt 3 ]; then
    echo -e One or more arguments missing!nUsage ./pgconfig.sh TOTAL_RAM NUM_USERS PGCONF_FILE
    exit 1
fi
...
MAX_CONNECTIONS=$((USERS40))
SHARED_BUFFERS=$((TOTAL_RAM4))
WORK_MEM=$((TOTAL_RAMUSERS8))
if [ $WORK_MEM -lt 128 ]; then
    WORK_MEM=128
fi
MAINTANANCE_WORK_MEM=$((TOTAL_RAM16))
...
# Test output
echo -e nThe following settings will be written to postgresql.conf
...
read -p Are you sure you want to use these postgres settings Type y or n  -n 1 -r
...
# Append new settings
echo -e nn###### CUSTOM SETTINGS OPTIMIZED FOR RASDAMAN GIVEN ${TOTAL_RAM} MB RAM AND ${USERS} USERS #######  $POSTGRES_CONFFILE
...
service postgresql restart & devnull
echo -e DONE