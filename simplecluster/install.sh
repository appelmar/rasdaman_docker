...
docker stop rasdaman-n1
docker stop rasdaman-n2
docker stop rasdaman-n3
docker rm rasdaman-n1
docker rm rasdaman-n2
docker rm rasdaman-n3
...
cd rasdaman-db
docker build --rm=false --tag="$IMAGE_TAG_DB" . && echo "Docker image $IMAGE_TAG_DB build successfully!"
...
docker run -d --name="rasdaman-n1" -h "rasdaman-n1"  -p 22201:22 -p 22231:8080 -p 22221:7001 $IMAGE_TAG 
